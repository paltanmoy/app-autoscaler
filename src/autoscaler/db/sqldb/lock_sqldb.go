...
func (ldb *LockSQLDB) Fetch() (lock *models.Lock, err error) {
	...
	fetchLockErr := ldb.sqldb.QueryRow(query).Scan(&owner, &timestamp, &ttl)
	if fetchLockErr != nil {
...
func (ldb *LockSQLDB) Acquire(lockDetails *models.Lock) (claimed bool, err error) {
	...
	currentTimestamp, timeErr := ldb.GetDatabaseTimestamp()
	if timeErr != nil {
		ldb.logger.Error("error-getting-timestamp-while-acquiring-lock", timeErr)
...
func (ldb *LockSQLDB) Renew(owner string) error {
	...
	currentTimestamp, timeErr := ldb.GetDatabaseTimestamp()
	if timeErr != nil {
		ldb.logger.Error("error-getting-timestamp-while-renewing-lock", timeErr)
		return timeErr
	}
...
func (ldb *LockSQLDB) Lock(lock *models.Lock) (bool, error) {
	...
	if err != nil && err == sql.ErrNoRows {
		...
		if fetchedLock.Owner == lock.Owner {
			...
			currentTimestamp, timeErr := ldb.GetDatabaseTimestamp()
			if timeErr != nil {
				ldb.logger.Error("error-getting-timestamp-while-getting-lock", timeErr)
				return false, timeErr
			}
